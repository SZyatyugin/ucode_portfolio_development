const nav_bar_sticky = () => {
    ...
    function check_coords_place_navbar() {
        ...
        if (window.pageYOffset > 20) {
            get(
                '.message'
            ).style.transform = `translateY(-${window.pageYOffset}px)`
        } else {
            get('.message').removeAttribute('style')
        }
    }

    get('.header_circle_button').addEventListener(
        'click',
        scroll_down_click_circle
    )
    ...
    function scroll_to_element(e) {
        if (e.target.className == 'menu_close' || e.target.className == '') {
            ...
            let clicked_menu_list = Object.values(getAll('.menu_list_items'))
                .find((elem) => {
